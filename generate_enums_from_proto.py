...
try:
	# Newer protobufs (where public path exists)
	from google.protobuf.enum_type_wrapper import EnumTypeWrapper  # type: ignore
except Exception:
	# 3.20.x and earlier
	try:
		...
		class EnumTypeWrapper:
			def __init__(self, enum_descriptor):
				self._enum = enum_descriptor
			def Name(self, number):
				return self._enum.values_by_number[number].name
			def Value(self, name):
				return self._enum.values_by_name[name].number
			def keys(self):
				return list(self._enum.values_by_name.keys())
			def values(self):
				return [v.number for v in self._enum.values]
			def items(self):
				return [(v.name, v.number) for v in self._enum.values]
